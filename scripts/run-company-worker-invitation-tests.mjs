...
const outputDirectory = resolve(".company-worker-invitation-runtime-test-dist");
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") throw new Error(`M1.10 runtime dependency escaped src/lib: ${sourcePath}`);
  return value;
}
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(`M1.10 runtime dependency could not be resolved: ${specifier} from ${importerPath}`);
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) throw new Error(`Unresolved M1.10 runtime alias: ${imported.fileName}`);
...
function collectRuntimeSources(entryFiles) {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(sourcePath)) fail(`M1.10 RED: required runtime production module is missing: ${relativePath}`);
...
function compileRuntimeModule(relativePath) {
  ...
  const compiled = ts.transpileModule(runtimeSource(sourcePath), {
    ...
    compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.CommonJS, esModuleInterop: true, strict: true, removeComments: false }
  });
  ...
  if (errors.length > 0) {
    ...
    fail(`M1.10 runtime compile failed for ${relativePath}`);
  }
