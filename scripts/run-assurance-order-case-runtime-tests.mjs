...
const outputDirectory = resolve(".assurance-order-runtime-test-dist");
const sourceRoot = resolve("src", "lib");
const LIB_ALIAS_PREFIX = "@/lib/";
rmSync(outputDirectory, { recursive: true, force: true });

const ENTRY_FILES = Object.freeze([
  "assurance/assurance-order-domain.ts",
  "assurance/assurance-order-repository.ts",
  "assurance/assurance-order-service.ts",
  "assurance/assurance-action-centre-service.ts"
]);
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") throw new Error(`M2.01 runtime dependency escaped src/lib: ${sourcePath}`);
  return value;
}
function resolveSourceImport(importerPath, specifier) {
  let base;
  if (specifier.startsWith(".")) base = resolve(dirname(importerPath), specifier);
  else if (specifier.startsWith(LIB_ALIAS_PREFIX)) base = resolve(sourceRoot, specifier.slice(LIB_ALIAS_PREFIX.length));
  else return null;
  ...
  throw new Error(`M2.01 runtime dependency could not be resolved: ${specifier} from ${importerPath}`);
}
function runtimeSpecifier(importerPath, dependencyPath) {
  let value = relative(dirname(importerPath), dependencyPath).replaceAll("\\", "/").replace(/\.ts$/, "");
  if (!value.startsWith(".")) value = `./${value}`;
  return value;
}
function runtimeSource(sourcePath) {
  let source = readFileSync(sourcePath, "utf8").replace(/^import "server-only";\r?\n\r?\n?/, "");
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) throw new Error(`Unresolved M2.01 runtime alias: ${imported.fileName}`);
...
function collectRuntimeSources(entryFiles) {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(sourcePath)) fail(`M2.01 runtime production module is missing: ${relativePath}`);
...
function compileRuntimeModule(relativePath) {
  ...
  if (errors.length > 0) {
    ...
    fail(`M2.01 runtime compile failed for ${relativePath}`);
  }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports, "__esModule", { value: true });\nexports.getDatabaseClient = async function getDatabaseClient() { throw new Error("M2.01 runtime test must inject a database client."); };\n',
  "utf8"
);

const tests = spawnSync(process.execPath, ["--test", resolve("tests", "platform", "assurance-order-case-runtime.test.mjs")], {
  stdio: "inherit",
  env: { ...process.env, HSE_ASSURANCE_ORDER_RUNTIME_DIST: outputDirectory }
});
rmSync(outputDirectory, { recursive: true, force: true });
