...
const out = resolve(".assessment-attempt-draft-repository-runtime-test-dist");
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`M2.08 draft repository dependency escaped src/lib: ${path}`);
  }
  return value;
}
...
function resolveImport(importer, specifier) {
  ...
  throw new Error(`M2.08 draft repository dependency unresolved: ${specifier} from ${importer}`);
}
...
function collect() {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(path)) fail(`M2.08 draft repository module missing: ${relativePath}`);
...
function compile(relativePath) {
  ...
  if (errors.length > 0) {
    ...
    fail(`M2.08 draft repository compile failed for ${relativePath}`);
  }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports,"__esModule",{value:true});\nexports.getDatabaseClient=async function(){throw new Error("M2.08 draft repository runtime injects its database client.");};\n',
  "utf8"
);

const tests = [
  resolve("tests", "platform", "assessment-attempt-draft-runtime.test.mjs"),
  resolve("tests", "platform", "assessment-attempt-draft-concurrency-runtime.test.mjs")
];
const result = spawnSync(process.execPath, ["--test", ...tests], {
  stdio: "inherit",
  env: { ...process.env, HSE_ASSESSMENT_ATTEMPT_DRAFT_REPOSITORY_RUNTIME_DIST: out }
});
rmSync(out, { recursive: true, force: true });
