...
const out = resolve(".assessment-catalogue-eligibility-test-dist");
const root = resolve("src", "lib");
const alias = "@/lib/";
const entries = [
  "assessment-catalogue/assessment-catalogue-domain.ts",
  "assessment-catalogue/assessment-catalogue-eligibility-service.ts"
];
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`M2.06 eligibility dependency escaped src/lib: ${path}`);
  }
  return value;
}
...
function resolveImport(importer, specifier) {
  ...
  throw new Error(`M2.06 eligibility dependency unresolved: ${specifier} from ${importer}`);
}
...
function collect() {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(path)) fail(`M2.06 eligibility runtime module missing: ${relativePath}`);
...
function compile(relativePath) {
  ...
  if (errors.length) {
    ...
    fail(`M2.06 eligibility runtime compile failed for ${relativePath}`);
  }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports,"__esModule",{value:true});\nexports.getDatabaseClient=async function(){throw new Error("M2.06 eligibility runtime injects its database client.");};\n',
  "utf8"
);

const result = spawnSync(
  process.execPath,
  ["--test", resolve("tests", "platform", "assessment-catalogue-eligibility-runtime.test.mjs")],
  {
    stdio: "inherit",
    env: { ...process.env, HSE_ASSESSMENT_CATALOGUE_ELIGIBILITY_RUNTIME_DIST: out }
  }
);
rmSync(out, { recursive: true, force: true });
