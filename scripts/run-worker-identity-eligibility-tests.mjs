...
const outputDirectory = resolve(".worker-identity-eligibility-runtime-test-dist");
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`Worker identity eligibility runtime dependency escaped src/lib: ${sourcePath}`);
  }
  return value;
}
...
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(
    `Worker identity eligibility runtime dependency could not be resolved: ${specifier} from ${importerPath}`
  );
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) {
      throw new Error(
        `Worker identity eligibility runtime alias could not be resolved: ${imported.fileName} from ${sourcePath}`
      );
    }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports, "__esModule", { value: true });\nexports.getDatabaseClient = async function getDatabaseClient() { throw new Error("Worker identity eligibility runtime test must inject a database client."); };\n',
  "utf8"
);
...
const tests = spawnSync(
  ...
  [
    "--test",
    resolve("tests", "identity", "worker-identity-eligibility-domain.test.mjs"),
    resolve("tests", "platform", "worker-identity-eligibility.test.mjs"),
    resolve("tests", "platform", "worker-identity-eligibility-migration-stack.test.mjs")
  ],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      HSE_WORKER_IDENTITY_ELIGIBILITY_RUNTIME_DIST: outputDirectory
    }
  }
);
