...
const out = resolve(".assessment-form-delivery-runtime-test-dist");
const root = resolve("src", "lib");
const alias = "@/lib/";
const entries = ["assessment-generation/assessment-form-delivery-service.ts"];
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") throw new Error(`M2.05 delivery dependency escaped src/lib: ${path}`);
  return value;
}
function resolveImport(importer, specifier) {
  ...
  throw new Error(`M2.05 delivery dependency unresolved: ${specifier} from ${importer}`);
}
...
function collect() {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(path)) fail(`M2.05 delivery runtime module missing: ${relativePath}`);
...
function compile(relativePath) {
  ...
  if (errors.length) {
    ...
    fail(`M2.05 delivery runtime compile failed for ${relativePath}`);
  }
...
writeFileSync(resolve(out, "database", "database.js"), '"use strict";\nObject.defineProperty(exports,"__esModule",{value:true});\nexports.getDatabaseClient=async function(){throw new Error("M2.05 delivery runtime injects its database client.");};\n', "utf8");
const result = spawnSync(process.execPath, ["--test", resolve("tests", "platform", "assessment-form-delivery.test.mjs")], {
  stdio: "inherit",
  env: { ...process.env, HSE_ASSESSMENT_FORM_DELIVERY_RUNTIME_DIST: out }
});
rmSync(out, { recursive: true, force: true });
