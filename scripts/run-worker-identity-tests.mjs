...
const outputDirectory = resolve(".worker-identity-runtime-test-dist");
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`Worker identity runtime dependency escaped src/lib: ${sourcePath}`);
  }
  return value;
}
...
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(
    `Worker identity runtime dependency could not be resolved: ${specifier} from ${importerPath}`
  );
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) {
      throw new Error(
        `Worker identity runtime alias could not be resolved: ${imported.fileName} from ${sourcePath}`
      );
    }
...
const tests = spawnSync(
  ...
  [
    "--test",
    resolve("tests", "identity", "worker-identity-domain.test.mjs"),
    resolve("tests", "platform", "worker-identity-foundation.test.mjs"),
    resolve("tests", "platform", "worker-identity-migration-stack.test.mjs")
  ],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      HSE_WORKER_IDENTITY_RUNTIME_DIST: outputDirectory
    }
  }
);
