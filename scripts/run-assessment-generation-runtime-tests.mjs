...
const out = resolve(".assessment-generation-runtime-test-dist");
...
const entries = [
  ...
  "assessment-generation/assessment-form-generation-service.ts"
];
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") throw new Error(`M2.05 runtime dependency escaped src/lib: ${path}`);
  return value;
}
