...
async function pageSnapshot(page, label, events) {
  ...
  const visibleAlert = await page.locator('[role="alert"], [role="status"]').filter({ visible: true }).allInnerTexts().catch(() => []);
...
async function loginRole(browser, credentials, role, label) {
  ...
  try {
    ...
    if (cred.totpSecret) {
      await page.getByLabel("Authenticator code").fill(totpCode(cred.totpSecret));
    }
    ...
    try {
      ...
      const snapshot = await pageSnapshot(page, `${label}-${role}-login-timeout`, events);
      throw new Error(`${role} login did not leave the login route under DOMContentLoaded semantics. ${error instanceof Error ? error.message : String(error)} Diagnostic=${JSON.stringify(snapshot)}`);
    }
...
await checkpoint("Supported latest-migration rollback and reapply uses the production database adapter", async () => {
  const rollbackDb = await openScriptDatabase({
...
try {
  for (const role of ROLES) {
    await checkpoint(`${role} repeated login is stable under DOMContentLoaded semantics`, async () => {
      const paths = [];
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        const session = await loginRole(browser, credentials, role, `repeat-${attempt}`);
        paths.push(session.finalPath);
        await session.context.close();
      }
      return { attempts: 2, paths };
    });
  }

  const isolationResults = {};
  for (const role of ROLES) {
    await checkpoint(`${role} runtime session is isolated from every other role dashboard`, async () => {
      const session = await loginRole(browser, credentials, role, "isolation");
      const probes = [];
      try {
        for (const target of ROLES.filter((candidate) => candidate !== role)) {
          ...
          if (status !== null && status >= 500) {
            add("critical", "cross-role-500", `/${target}/dashboard`, `${role}→${target} returned HTTP ${status}.`);
          }
...
const repeatedLoginPassed = ROLES.every((role) => checkpoints.some(
  (item) => item.name === `${role} repeated login is stable under DOMContentLoaded semantics` && item.status === "PASS"
));
const runtimeIsolationPassed = ROLES.every((role) => checkpoints.some(
  (item) => item.name === `${role} runtime session is isolated from every other role dashboard` && item.status === "PASS"
));
