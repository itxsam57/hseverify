...
const outputDirectory = resolve(".worker-identity-check-runtime-test-dist");
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`Worker identity check runtime dependency escaped src/lib: ${sourcePath}`);
  }
  return value;
}
...
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(
    `Worker identity check runtime dependency could not be resolved: ${specifier} from ${importerPath}`
  );
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) {
      throw new Error(
        `Worker identity check runtime alias could not be resolved: ${imported.fileName} from ${sourcePath}`
      );
    }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports, "__esModule", { value: true });\nexports.getDatabaseClient = async function getDatabaseClient() { throw new Error("Worker identity check runtime test must inject a database client."); };\n',
  "utf8"
);
...
const tests = spawnSync(
  ...
  [
    "--test",
    resolve("tests", "identity", "worker-identity-check-domain.test.mjs"),
    resolve("tests", "platform", "worker-identity-automated-checks.test.mjs"),
    resolve("tests", "platform", "worker-identity-automated-checks-migration-stack.test.mjs")
  ],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      HSE_WORKER_IDENTITY_CHECK_RUNTIME_DIST: outputDirectory
    }
  }
);
