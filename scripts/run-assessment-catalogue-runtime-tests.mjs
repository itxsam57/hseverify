...
const out = resolve(".assessment-catalogue-runtime-test-dist");
const root = resolve("src", "lib");
const alias = "@/lib/";
const entries = [
  "assessment-catalogue/assessment-catalogue-domain.ts",
  "assessment-catalogue/assessment-catalogue-service.ts"
];
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`M2.06 runtime dependency escaped src/lib: ${path}`);
  }
  return value;
}
...
function resolveImport(importer, specifier) {
  ...
  throw new Error(`M2.06 runtime dependency unresolved: ${specifier} from ${importer}`);
}
...
function collect() {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(path)) fail(`M2.06 runtime module missing: ${relativePath}`);
...
function compile(relativePath) {
  ...
  if (errors.length) {
    ...
    fail(`M2.06 runtime compile failed for ${relativePath}`);
  }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports,"__esModule",{value:true});\nexports.getDatabaseClient=async function(){throw new Error("M2.06 runtime injects its database client.");};\n',
  "utf8"
);
