...
const outputDirectory = resolve(".evidence-review-runtime-test-dist");
const sourceRoot = resolve("src", "lib");
const LIB_ALIAS_PREFIX = "@/lib/";
rmSync(outputDirectory, { recursive: true, force: true });

const ENTRY_FILES = Object.freeze([
  "review/evidence-review-domain.ts",
  "review/evidence-review-service.ts"
]);
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") throw new Error(`M2.02 runtime dependency escaped src/lib: ${sourcePath}`);
  return value;
}
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(`M2.02 runtime dependency could not be resolved: ${specifier} from ${importerPath}`);
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) throw new Error(`Unresolved M2.02 runtime alias: ${imported.fileName}`);
...
function collectRuntimeSources(entryFiles) {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(sourcePath)) fail(`M2.02 runtime production module is missing: ${relativePath}`);
...
function compileRuntimeModule(relativePath) {
  ...
  if (errors.length > 0) {
    ...
    fail(`M2.02 runtime compile failed for ${relativePath}`);
  }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports, "__esModule", { value: true });\nexports.getDatabaseClient = async function getDatabaseClient() { throw new Error("M2.02 runtime test must inject a database client."); };\n',
  "utf8"
);

const tests = spawnSync(process.execPath, ["--test", resolve("tests", "platform", "evidence-review-queue-runtime.test.mjs")], {
  stdio: "inherit",
  env: { ...process.env, HSE_EVIDENCE_REVIEW_RUNTIME_DIST: outputDirectory }
});
rmSync(outputDirectory, { recursive: true, force: true });
