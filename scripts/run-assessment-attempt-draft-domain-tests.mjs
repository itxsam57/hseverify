...
const out = resolve(".assessment-attempt-draft-runtime-test-dist");
const root = resolve("src", "lib");
const alias = "@/lib/";
const entries = ["assessment-attempt/assessment-attempt-draft-domain.ts"];
...
function relativeLib(path) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`M2.08 draft runtime dependency escaped src/lib: ${path}`);
  }
  return value;
}
...
function resolveImport(importer, specifier) {
  ...
  throw new Error(`M2.08 draft runtime dependency unresolved: ${specifier} from ${importer}`);
}
...
function collect() {
  ...
  function visit(relativePath) {
    ...
    if (!existsSync(path)) fail(`M2.08 draft runtime module missing: ${relativePath}`);
...
function compile(relativePath) {
  ...
  if (errors.length > 0) {
    ...
    fail(`M2.08 draft runtime compile failed for ${relativePath}`);
  }
...
const testFile = resolve("tests", "platform", "assessment-attempt-draft-domain-runtime.test.mjs");
const result = spawnSync(process.execPath, ["--test", testFile], {
  stdio: "inherit",
  env: { ...process.env, HSE_ASSESSMENT_ATTEMPT_DRAFT_RUNTIME_DIST: out }
});
rmSync(out, { recursive: true, force: true });
