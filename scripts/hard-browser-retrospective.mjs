...
try {
  ...
  await checkpoint("Worker evidence records preserve history through visible workflow", async () => {
    assert(workerPage, "Worker page is unavailable after profile/identity checkpoint");
    const errors = trackErrors(workerPage, "worker-evidence-history");
    const pdf = Buffer.from("%PDF-1.4\n1 0 obj\n<< /Type /Catalog >>\nendobj\ntrailer\n<< /Root 1 0 R >>\n%%EOF\n", "utf8");

    await gotoOk(workerPage, "/worker/evidence", "Qualifications, experience, employment and skills");
    await workerPage.getByLabel("Record type").selectOption("qualification");
    await workerPage.getByRole("button", { name: "Create draft" }).click();
    await waitForStatus(workerPage, "Qualification draft created.");

    const qualification = workerPage.getByRole("article").filter({ hasText: "Qualification record" }).first();
    await qualification.getByLabel("Qualification title").fill("NEBOSH International General Certificate");
    await qualification.getByLabel("Category").fill("Occupational Health and Safety");
    await qualification.getByLabel("Issuing organization").fill("NEBOSH");
    await qualification.getByLabel("Certificate / candidate number").fill("RETRO-QA-001");
    await qualification.getByLabel("Level").fill("Level 3");
    await qualification.getByLabel("Issue date").fill("2025-01-15");
    await qualification.getByLabel("Country").fill("United Kingdom");
    await qualification.getByLabel(/I confirm these qualification details/).check();
    await qualification.getByRole("button", { name: "Save metadata" }).click();
    await waitForStatus(workerPage, "Draft metadata saved with its current version.");

    const currentQualification = workerPage.getByRole("article").filter({ hasText: "Qualification record" }).first();
    await currentQualification.getByLabel("Primary certificate file").setInputFiles({
      name: "retro-qualification.pdf",
      mimeType: "application/pdf",
      buffer: pdf
    });
    await currentQualification.getByRole("button", { name: "Upload file" }).click();

    const immediateBound = workerPage.getByRole("status").filter({ hasText: "Certificate uploaded, security-scanned and bound" });
    const queued = workerPage.getByRole("status").filter({ hasText: "queued for security scanning" });
    await Promise.race([
      immediateBound.first().waitFor({ state: "visible", timeout: 15_000 }),
      queued.first().waitFor({ state: "visible", timeout: 15_000 })
    ]);
    if (await queued.first().isVisible().catch(() => false)) {
      for (let attempt = 0; attempt < 4; attempt += 1) {
        const scanButton = workerPage.getByRole("button", { name: "Check scan status" }).first();
        if (!(await scanButton.isVisible().catch(() => false))) break;
        await scanButton.click();
        const attached = workerPage.getByRole("status").filter({ hasText: "Security scan passed and the evidence file is now attached" }).first();
        if (await attached.isVisible().catch(() => false)) break;
        await workerPage.waitForTimeout(250);
      }
    }

    const submittedQualification = workerPage.getByRole("article").filter({ hasText: "Qualification record" }).first();
    await submittedQualification.getByRole("button", { name: "Submit this version" }).click();
    await waitForStatus(workerPage, "Evidence version submitted. Its accepted history is now immutable.");
    await workerPage.reload({ waitUntil: "domcontentloaded" });

    const afterSubmit = workerPage.getByRole("article").filter({ hasText: "Qualification record" }).first();
    const submitText = await afterSubmit.innerText();
    assert(submitText.includes("submitted"), "Qualification did not remain submitted after hard reload");
    assert(submitText.includes("retro-qualification.pdf"), "Uploaded PDF was not visible in qualification history after reload");
    assert(submitText.includes("Version 1"), "Submitted qualification version 1 was missing from visible history");

    await afterSubmit.getByRole("button", { name: "Start a new revision" }).click();
    await waitForStatus(workerPage, "A new editable version was created. The submitted version remains preserved in history.");
    await workerPage.reload({ waitUntil: "domcontentloaded" });

    const afterRevision = workerPage.getByRole("article").filter({ hasText: "Qualification record" }).first();
    const historyText = await afterRevision.innerText();
    assert(historyText.includes("Version 1"), "Prior submitted qualification version disappeared after starting a revision");
    assert(historyText.includes("Version 2"), "New qualification revision was not visible after hard reload");
    assert(historyText.includes("retro-qualification.pdf"), "Prior PDF history disappeared after creating a new revision");
    await workerPage.screenshot({ path: `${artifactsDir}/worker-evidence-version-history.png`, fullPage: true, caret: "initial" });

    assert(errors.length === 0, `Worker evidence/history browser errors: ${errors.join(" | ")}`);
    return { record: "qualification", pdf: "retro-qualification.pdf", priorVersionPreserved: true };
  });
