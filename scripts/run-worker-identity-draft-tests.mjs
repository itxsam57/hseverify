...
const outputDirectory = resolve(".worker-identity-draft-runtime-test-dist");
...
const ENTRY_FILES = Object.freeze([
  ...
  "identity/worker-identity-draft-service.ts"
]);
...
function normalizeRelativeSourcePath(sourcePath) {
  ...
  if (value.startsWith("../") || value === "..") {
    throw new Error(`Worker identity draft runtime dependency escaped src/lib: ${sourcePath}`);
  }
  return value;
}
...
function resolveSourceImport(importerPath, specifier) {
  ...
  throw new Error(
    `Worker identity draft runtime dependency could not be resolved: ${specifier} from ${importerPath}`
  );
}
...
function runtimeSource(sourcePath) {
  ...
  for (const imported of preprocessed.importedFiles) {
    ...
    if (!dependencyPath) {
      throw new Error(
        `Worker identity draft runtime alias could not be resolved: ${imported.fileName} from ${sourcePath}`
      );
    }
...
writeFileSync(
  ...
  '"use strict";\nObject.defineProperty(exports, "__esModule", { value: true });\nexports.getDatabaseClient = async function getDatabaseClient() { throw new Error("Worker identity draft runtime test must inject a database client."); };\n',
  "utf8"
);
...
const tests = spawnSync(
  ...
  [
    "--test",
    resolve("tests", "identity", "worker-identity-draft-domain.test.mjs"),
    resolve("tests", "platform", "worker-identity-draft.test.mjs"),
    resolve("tests", "platform", "worker-identity-draft-migration-stack.test.mjs")
  ],
  {
    stdio: "inherit",
    env: {
      ...process.env,
      HSE_WORKER_IDENTITY_DRAFT_RUNTIME_DIST: outputDirectory
    }
  }
);
