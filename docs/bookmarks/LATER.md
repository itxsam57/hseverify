...
|---|---|---|---|---|---|
| LATER-004 | M1.02 | Portal-wide design system | Implementation complete — owner retest required | PR #8 implements shared tokens, reusable controls, live route adoption, mobile navigation, dialog/table contracts and automated validation. The Windows owner tests found `LATER-OWNER-003` in preview-bundle creation and `LATER-OWNER-004` in shared runtime/build output. | Close after the focused Windows runtime/build/preview retest and remaining M1.02 browser acceptance pass. |
...
| LATER-045 | Current owner gate | M1.02 design-system responsive, accessibility, runtime/build and preview acceptance | Implementation complete — owner retest required | The owner passed the application checks through the protected runtime smoke. The production build then failed on a malformed `.next/dev/types/validator.ts` created after standalone typecheck. The Windows preview repair also still requires owner confirmation. | Close after `docs/testing/M1_02_RUNTIME_BUILD_RETEST.md` and remaining M1.02 browser checks report Overall PASS. |
| LATER-OWNER-003 | M1.02 | Windows standalone preview copy attempted a privileged PGlite symlink | Owner defect — implementation fixed, retest pending | On Windows/Node 22.23.1, `npm run preview:smoke` failed with `EPERM: operation not permitted, symlink` before `server.js` started. PR #9 materializes traced links as ordinary files, cleans partial bundles, verifies no links remain, verifies PGlite inclusion and adds a portable-copy regression. The owner reached the repaired preview retest only after the complete build gate, which was blocked by `LATER-OWNER-004`. | Pass the preview sections in `docs/testing/M1_02_RUNTIME_BUILD_RETEST.md` from a normal Command Prompt with Developer Mode unnecessary, successful routes, clean repeatability and confirmed shutdown. |
| LATER-OWNER-004 | M1.02 | Protected runtime smoke corrupted generated Next development types consumed by production build | Owner defect — implementation fixed, retest pending | On Windows/Node 22.23.1, `npm run check` passed standalone typecheck and every stage through `test:runtime-db`, then `next build` failed on `.next/dev/types/validator.ts` line 89 with `Cannot find name 'er'`. The runtime smoke had launched `next dev` in the shared `.next` directory after standalone typecheck. PR #10 isolates it in `.next-runtime-smoke`, cleans stale development output before typecheck/build, terminates the Windows process tree, forces the production output directory and adds the exact malformed-validator regression. | Pass the full Windows runtime/build/preview owner retest with clean tracked files and no generated-type, process, persistence or preview regression. |
