...
|---|---|---|---|---|---|
| LATER-004 | M1.02 | Portal-wide design system | Implementation complete — owner retest required | PR #8 implements shared tokens, reusable controls, live route adoption, mobile navigation, dialog/table contracts and automated validation. Windows owner tests found `LATER-OWNER-003` through `LATER-OWNER-008` in preview portability, generated-output isolation, tracked source determinism, the ordinary development command, Worker Profile page-width containment and platform-stable validation. | Close after `docs/testing/M1_02_DEVELOPMENT_CONFIG_RETEST.md`, `docs/testing/M1_02_PROFILE_OVERFLOW_RETEST.md`, the remaining sections of `docs/testing/M1_02_FULL_REWRITE_RETEST.md` and all M1.02 browser/accessibility sections report Overall PASS. |
...
| LATER-045 | Current owner gate | M1.02 design-system responsive, accessibility, deterministic development/build and preview acceptance | Implementation complete — owner retest required | The owner passed the rewritten malformed-validator recovery and full application gate, then found tracked TypeScript configuration changes. PR #11 protected automated modes; PR #12 protected ordinary development. The next owner pass found page-wide horizontal overflow on Worker Profile. PR #13 rewrote shell/Profile width containment. The first Windows retest then exposed an LF-only assertion in the new regression; PR #14 makes the contract platform-stable and adds synthetic CRLF coverage. Windows confirmation and remaining browser acceptance remain mandatory. | Close after `docs/testing/M1_02_DEVELOPMENT_CONFIG_RETEST.md`, `docs/testing/M1_02_PROFILE_OVERFLOW_RETEST.md`, the remaining full rewrite sections and all M1.02 browser checks report Overall PASS. |
...
| LATER-OWNER-007 | M1.02 | Worker Profile created page-wide horizontal overflow and clipped action controls | Owner defect — implementation fixed, retest pending | On Windows 10/Chrome/Node 22.23.1 at normal desktop width and 100% zoom, `/worker/profile` widened the document, moved the complete shell/sidebar during horizontal scrolling, clipped Ready to submit and left Submit profile partly outside the viewport. Root causes were viewport-based Profile breakpoints despite a fixed sidebar, a fixed 300px action-column minimum and incomplete shrink containment through shell/Profile/table ancestors. PR #13 introduces shell width containment, Profile container queries, a shrinkable 16rem–22rem action track, bounded controls, table-local scrolling and permanent overflow contracts. | Pass `docs/testing/M1_02_PROFILE_OVERFLOW_RETEST.md` at normal desktop, 860px, 768px, 390px, 320px and 125%/150%/200% zoom with no document overflow and table-only horizontal scrolling. |
| LATER-OWNER-008 | M1.02 | Profile overflow regression failed on Windows CRLF despite correct CSS | Owner defect — implementation fixed, retest pending | On Windows 10/Node 22.23.1, `npm run test:profile-overflow` passed 3 tests and failed the history-scroll contract because the test searched for an exact LF-only multiline CSS string. The checked-out CSS used CRLF and contained the required `.profile-history-card { overflow: hidden; }` declaration. PR #14 normalizes source line endings, inspects CSS selector/declaration semantics and adds a synthetic CRLF regression so Linux CI exercises the Windows path. | Rerun `npm run test:profile-overflow` on Windows and require 5 pass / 0 fail, then continue the complete Profile overflow owner matrix. |
