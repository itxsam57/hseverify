...
|---|---|---|---|---|---|
| LATER-005 | M1.03 | Real Worker registration | Not started | Current access still uses environment-gated demonstration credentials. | Complete in M1.03. |
| LATER-006 | M1.03 | Mandatory email OTP | Not started | No persisted secure OTP workflow exists. | Complete in M1.03 with hashing, expiry, replay prevention, attempt limits and resend controls. |
| LATER-007 | M1.03 | Mandatory phone OTP | Not started | No persisted SMS OTP adapter/workflow exists. | Build and owner-test a sandbox workflow in M1.03; live activation remains under LATER-036. |
| LATER-008 | M1.03 | Password reset, recovery and account lifecycle | Not started | Demonstration authentication has no production account lifecycle. | Complete in M1.03. |
| LATER-009 | M1.03/M1.08 | Company registration and role-specific authentication | Not started | Only Worker demonstration access exists. | Authentication foundation in M1.03; Company verification in M1.08. |
| LATER-010 | M1.03 | Reviewer, assessor, administrator and root provisioning/MFA | Not started | Staff activation, MFA and protected staff portals do not exist. | Complete provisioning and portal guards in M1.03; operational modules follow later. |
...
| LATER-013 | M1.04 | Cross-role/cross-tenant direct-endpoint security suite | Partial | Worker-only checks exist; the full portal and tenant matrix does not. | Complete and expand cumulatively in M1.04. |
| LATER-014 | M1.05 | Immutable platform audit engine | Partial | Profile-level audit behavior exists; the platform audit store does not. | Complete in M1.05. |
...
| LATER-026 | M1.07 | Duplicate-worker detection and controlled merge review | Not started | Current identity derives from demonstration session values. | Complete in M1.07. |
| LATER-027 | M1.07 | Permanent Worker ID issuance transaction | Partial | A demonstration Worker ID exists, but no accepted-submission issuance transaction exists. | Complete in M1.07. |
| LATER-028 | M1.07/M2.02 | Identity verification states and retained versions | Not started | Profile correction requests exist, but identity submission/review history does not. | Complete Worker states in M1.07 and reviewer queue behavior in M2.02. |
...
| LATER-031 | M1.10 | Worker/staff invitations and Company codes | Not started | Invitation and token lifecycle does not exist. | Complete in M1.10. |
