# M1.05 Subunit 2 Owner Hard Test
...
- `0008_transactional_outbox_jobs` is applied.

## C. Focused outbox gate
...
1. only `0008_transactional_outbox_jobs` rolls back;
2. migrations `0001` through `0007` remain applied;
3. `0008` becomes pending;
4. immutable outbox lifecycle audit facts remain valid;
5. migration reapplication succeeds;
6. all migrations return to applied with matching checksums.
...
No browser workflow is required because this subunit has no visible product surface.
