...
Make an active Worker assessment resilient to reloads, crashes, connectivity loss, session interruption, technical problems, and emergency exits without weakening any accepted M2.07 guarantees.

A Worker must be able to type an uncommitted answer, have that work protected automatically, leave or lose the assessment window safely, and recover the correct current question and latest recoverable draft. Autosave must never be mistaken for answer submission. `Next` and final `Submit assessment` remain the only operations that commit an answer and progress the immutable attempt.
...
11. audit/timeline evidence for recovery events without answer content;
...
- **server draft**: authoritative latest accepted uncommitted answer for the current position;
...
- `questionVersionId`;
- `questionType`;
- one canonical typed answer value using the same six accepted M2.07 answer types;
- monotonically increasing `revision`;
- the last accepted mutation/idempotency key;
- `createdAt`;
- `updatedAt`.

The six answer types remain the existing M2.07 types:
...
Draft values use the same normalization and validation rules as committed values. An invalid draft is rejected rather than stored in an alternate looser format.
...
Draft answer content must not appear in generic audit metadata, timeline metadata, logs, technical-issue records, exception messages, analytics, or telemetry.
...
A draft save accepts only the minimum stale-request guards and answer payload needed for the owning Worker, including:

- attempt reference;
- echoed current position/question-version reference;
- normalized-answer input;
...
6. normalize/validate the answer using the pinned question type/options;
7. compare the supplied expected revision with the current server draft revision;
8. make an exact retry of the same mutation key idempotent;
9. reject a reused mutation key carrying different content;
...
4. normalize the explicit current answer or the exact authoritative draft selected for commit;
...
- optional pending technical-issue metadata that contains no answer text.
...
- exact retries with the same mutation key converge idempotently;
- a different payload reusing the same mutation key fails closed;
- a stale tab cannot commit a different question after another tab has advanced the attempt;
- unsynced local text is not silently discarded when a newer server draft exists.
...
- a bounded Worker description;
- attempt reference and safe position metadata added server-side;
- timestamps/idempotency metadata.

The report must reject or strip answer content, answer keys, rubrics, screenshots/video/audio, scoring data, arbitrary hidden form data, or raw client diagnostics that could leak sensitive assessment content. M2.09 may later own richer integrity/evidence capture; M2.08 does not.
...
The interrupted predecessor remains historical evidence and cannot later resume concurrently once a successor is established. The Assurance Case's active assessment reference moves atomically to the authorized successor.
...
- If a session disappears unexpectedly, encrypted local material may remain unread as a safety buffer but is not displayed until the server revalidates the same owning Worker/attempt/current question.
...
No M2.08 response, HTML, browser state, local recovery object, technical-issue report, audit record, or error message may add any of the M2.07 forbidden data:
...
- revision/idempotency data for draft CAS;
- interruption/recovery lifecycle support;
- bounded technical-issue records without answer content;
...
- draft typed values follow the same type integrity as committed values;
...
- replacement activation cannot leave two concurrently active assessment attempts for the same recovery lineage.
...
Audit/timeline events must never include candidate answer content or local ciphertext/key material.
...
- valid draft autosave for all six answer types;
- invalid values rejected using existing normalization rules;
...
- exact idempotent retry converges;
- reused idempotency key with different content fails;
...
- session expiry/revocation blocks recovery mutations until valid reauth;
- submitted attempt cannot enter recovery lifecycle.

### Technical issue reporting

- report-and-continue records bounded metadata and leaves lifecycle unchanged;
- report-and-exit protects draft, records/queues report, and exits;
- offline report retry is idempotent;
- report/audit payloads contain no answer content or forbidden assessment secrets.
...
M2.08 is complete only when a real authenticated Worker can type on the current question and recover that uncommitted work through refresh/crash/connectivity interruption without it becoming a committed answer; server draft state arbitrates cross-device recovery and stale writers; device-local encryption protects unsynced same-device work; Emergency Exit and Report Technical Issue are functional even under failure conditions; same-form recovery preserves immutable M2.07 history by default; exceptional replacement preserves lineage and the strongest never-repeat rule; submitted/foreign/superseded attempts cannot be revived; no recovery surface leaks hidden assessment/scoring data; and the exact implementation head passes targeted RED-derived tests, real browser regression, the full Engineering Gate, and the permanent Independent full-system audit.
